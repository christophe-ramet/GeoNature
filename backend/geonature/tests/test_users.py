...
class TestUsers:
    ...
    def test_delete_organism(self, users):
        ...
        create_response = self.client.post(url_for("users.create_organism"), json=new_organism_data)
        assert create_response.status_code == 200
        organism_id = create_response.json["id_organisme"]

        # Now delete it
        # url_for may generate duplicate /geonature/api prefix, so we deduplicate it
        delete_url = url_for("users.delete_organism", id_organisme=organism_id).replace(
            "/geonature/api/geonature/api", "/geonature/api"
        )
        ...
        # url_for may generate duplicate /geonature/api prefix, so we deduplicate it
        get_url = url_for("users.get_organism", id_organisme=organism_id).replace(
            "/geonature/api/geonature/api", "/geonature/api"
        )
